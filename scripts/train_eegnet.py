...
def main():
    ...
    # Load data
    X_train, _, y_train, _ = load_npz_split(train_dataset)
    X_test, _, y_test, _ = load_npz_split(test_dataset)
    train_loader, test_loader = _create_loaders(X_train, y_train, X_test, y_test)

    # Build model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = EEGNet(n_channels=21, n_classes=4, sfreq=128).to(device)
...
def _create_loaders(X_train, y_train, X_test, y_test):
...
def train_loop(model, train_loader, test_loader, class_weights, num_epochs, device, out_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_raw, labels in train_loader:
        ...
        with torch.no_grad():
            for batch_raw, labels in test_loader:
                batch_raw, labels = batch_raw.to(device), labels.to(device)
        ...
        if epoch_val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), out_dir / "EEGNet.pth")
